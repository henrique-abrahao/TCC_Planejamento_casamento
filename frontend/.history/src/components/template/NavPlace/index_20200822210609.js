...
export default class NavPlace extends Component {
  ...
  renderCard = (name, image) => {
    return (
      <Card onMouseEnter={() => this.setState({ width: '20%' })} onMouseLeave={() => this.setState({ width: '15%' })} className="text-center" style={{ width: this.state.width }} >
        <Card.Img variant="top" src={image} style={{ height: 150, objectFit: 'cover' }} />
