...
routes.post("/login", async (req, res) => {
  ...
  let user = await knex('cliente').select({ name: 'nome_noiva' }, { email: 'email_noiva' }, { data: 'data_casamento' }).where('email_noiva', req.body.email).andWhere('password', req.body.pass);
  if (user.length) return res.json(user);
  user = user = await await knex('cliente').select({ name: 'nome_noivo' }, { email: 'email_noivo' }, { data: 'data_casamento' }).where('email_noivo', req.body.email).andWhere('password', req.body.pass);
  if (user.length) return res.json(user);
  return res.status(400).json({ 'message': 'email or password invalid' })
})

routes.post("/guestList", async(req, res) => {
  console.log(req.body);
  try{
    await knex('cliente').update({
      number_guest: req.body.number_guest
    }).where('email_noiva', req.body.email).orWhere('email_noivo', req.body.email);
...
routes.get('/photo', async (req, res) => {
  const img = await knex('photo').select('*').innerJoin('images', 'photo.id', 'images.photo_id');
...
routes.delete("/deleta", async(req, res) => {
  await knex('photo').delete
})
