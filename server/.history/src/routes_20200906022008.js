...
routes.post("/login", async (req, res) => {
  ...
  let user = await knex.column('nome_noiva',{by: 'name'}, 'email_noiva', 'data_casamento').select().from('cliente').where('email_noiva', req.body.email).andWhere('password', req.body.pass);
  if (user.length) return res.json(user);
  user = await knex('cliente').select('nome_noivo', 'email_noivo', 'data_casamento').where('email_noivo', req.body.email).andWhere('password', req.body.pass);
  if (user.length) return res.json(user);
